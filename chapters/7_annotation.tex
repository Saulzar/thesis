...
% The human verification threshold defines both the average level of variation for annotations produced by any verification based annotation tool, and the level of assistance it provides by reducing the burden of the annotator (in combination with the prediction accuracy of the object detector). For a very low, strict, threshold a human annotator must  
...
\section {Dataset specific discussion}
\label{sec:dataset_discussion}

Here I discuss some aspects relating to particular datasets. Note that appendix~\ref{chap:datasets} contains figures which give a more detailed breakdown of the annotation process for each dataset.
 


\subsection{ Apples to apples comparison }
