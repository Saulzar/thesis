...
After noticing over-fitting occurring in the noisy cases a the set of experiments were repeated with reduced levels of data, firstly at $25\%$ of training examples, then at $6.25\%$, in order to examine how noise affected generalisation with 
...
The impact in terms of degradation of object detection performance can be seen in table~\ref{tab:noise_table_100}, measured as the reduction in performance from the zero noise, zero offset case. Despite having a similar mean \gls{IOU} overlap with the original boxes for a similar parameter magnitude, the systematic offset degraded performance more at strict matching threshold $AP_{75}$, whereas noise had more impact at relaxed matching thresholds $AP_{50}$. 
...
% In this experiment I test the hypothesis that an object detector trained with single-class training learns faster than multi-class training. For this, the Pascal VOC and Microsoft COCO datasets are used. I use two subsets of images  each comprising a set of images containing any object in a set of four classes. First an object detector is trained in the multi-class scenario on all four classes, afterwards each of the four classes is trained individually. 
