...
\toprule
$\sigma=0\%$          & $0.0\pm0.0$  & $0.6\pm0.4$  & $0.8\pm0.5$  & $1.6\pm1.2$  & $7.9\pm8.9$   \\
...
$\sigma=0\%$                            & $0.0\pm0.0$  & $0.7\pm0.4$  & $0.8\pm0.6$  & $3.0\pm2.1$  & $10.2\pm10.7$ \\
...
At relaxed matching thresholds performance is also much less degraded overall, for example $AP_{50}$ is only degraded $10\%$ at $\sigma = 16\%, \Delta = 16\%$, yet in the same case $AP_{75}$ is reduced by $71\%$. Perhaps this should not be surprising given the mean \gls{IOU} overlap in this case is only $49.0$. What is more surprising is the robustness of the object detector at the more relaxed matching threshold. 

Noise at $\sigma = 32\%$ was much worse in all cases, but this can be considered a special case. Using a normally distributed scaling for $\sigma = 32\%$ was probably an incorrect choice, as it can result in boxes of significantly different aspect ratio to the original box. Adding noise to the corner positions may have been more appropriate at this high noise level.
...
Over-fitting can be seen to occur at higher levels and offset, where the validation scores peak and start to slowly degrade with further training. In low-data settings, for example at the very start of an annotation process, the risk over over-fitting is much higher. Therefore it seems likely that with much less data that both noisy annotations, and those with systematic bias will be more of a problem. It would be an interesting data-point to verify this by repeating the same experiment using only a fraction of the images.
