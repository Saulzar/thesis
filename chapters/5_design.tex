...
Many image datasets are either captured across time, or are derived from video. Of annotated datasets described in this work (see chapter \ref{chap:annotation}) in this work approximately half are derived from video, and of those remaining two are time series. Functionality for annotating of video sequences is limited in this work. Organisation of images in order of time (and frame sequence) is provided, and the ability to view a small amount of context in time around a current frame is supported (by use of shortcut keys).
...
\subsection{Dual threshold method}
...
In object detection datasets, many images contain a mixture of easy and difficult examples, so it makes sense to use both active learning and a verification based annotation.  First active learning example selection picks images containing uncertain object detections, then a verification based annotation approach is taken to annotate the uncertain image, where the user needs only annotate or correct the hard cases where the model performs badly.

I have implemented a number of simple metrics for selecting images based on heuristics which I have found useful in different kinds of workflows. One use case is sparsely populated images, For example in the scallop imagery to best utilise annotator time it seems prudent to present images containing (or believed to be contain) scallops first. Another related use case is annotating a single class (or few classes) from a much larger image set, where only a subset of the images contains the object in question.

There exist very recently published measures of uncertainty for object detection (for example using Bayesian \gls{NMS} \cite{Harakeh}, ensembles \cite{Le2018}, or test time augmentation \cite{Wei2018}), a future direction will be to integrate uncertainty approaches; using cross-validation with an ensemble of models seems like a promising direction, although it will increase the training time.
...
In general evaluating the entire image set of unlabelled images for each training iteration is generally far too slow, and using a random sampling also ensures a more balanced set of training examples, where reliance too heavily on a heuristic may result in many images similar images being selected and causing an imbalance in the training set.

For certain kinds of heuristic, such as frame variation (discussed below), it is necessary to evaluate many frames in sequence. For this there currently exists a manual operation where a user can schedule a full evaluation of the set of images, which can occur much less frequently.
